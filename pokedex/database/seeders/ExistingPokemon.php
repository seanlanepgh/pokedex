...
class ExistingPokemon extends Seeder
{
    ...
    private function allPokemon()
    {
        ...
        while (! feof($file)) {
            ...
            foreach ($this->value as $v)
                // attach the abilities to the pokemon by getting the id of the ability with the name specified by $v. 
    ...
    private function insertPokemon($line)
    {
        DB::table('pokemon')->insert([
            'id' => (int) $line[0],
            'name' => (string) $line[1],
            'sprite' => (string) "https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/" . $line[0] . ".png",
            'height' => (float) $line[3],
            'weight' => (float) $line[4],
            'stats' => (string) $line[7],
            'species' => (string) $line[8],
